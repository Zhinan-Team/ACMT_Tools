...
from dvadmin.system.models import (
    Role, Users, Rule, Menu, MenuButton, Parameter, SystemConfig,
...
class UsersInitSerializer(CustomModelSerializer):
    ...
    class Meta:
        model = Users
        fields = ["username", "email", 'mobile', 'avatar', "name", 'gender', 'user_type', 'user_type',
                  'first_name', 'last_name', 'email', 'is_staff', 'is_active', 'creator',
...
class ParameterInitSerializer(CustomModelSerializer):
    """
    初始化获取数信息(用于生成初始化json文件)
    """
    ...
    class Meta:
        model = Parameter
        fields =  ["name", "guid", "attributes", "state", "sample_frame", "sample_rate", "updated_timeout", "record_rate", "recording_data_group", "global_guid"]
        read_only_fields = ["id"]

class RuleInitSerializer(CustomModelSerializer):
    """
    初始化获取数信息(用于生成初始化json文件)
    """
    ...
    class Meta:
        model = Rule
        fields =  ["name","guid", "group_info", "rate", "delay", "trigger", "init_state", "state_filter", "trigger_limit", "log_limit", "aircraft_part","condition", "event_record", "task_id"]
        read_only_fields = ["id"]
